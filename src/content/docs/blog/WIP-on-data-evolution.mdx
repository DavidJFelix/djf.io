...
subtitle: 'Schemas, types, structures, and versioning data -- what more could you want?'
author: DavidJFelix
description: 'Schemas, types, structures, and versioning data -- what more could you want?'
date: 2023-12-30
tags:
  - data
  - types
...
readingTime: 5m
hero:
  tagline: 'Schemas, types, structures, and versioning data -- what more could you want?'
  image:
...
<Tabs>
  <TabItem label="TypeScript">
    ...
    Typescript allows us to do compile/transpile-time type-checking and provides a few niceties for runtime type-assertions.
    This is a thin wrapper over the capabilities of Javascript.
    Even after we compile this code and the types disappear, the data still has structure.
  ...
  <TabItem label="SQLite">
    ...
    In addition to being a query language, SQL is also a data definition language (DDL).
    The DDL capabilities allow us to add strong types to our data.
  ...
  <TabItem label="Protobuf">
    ...
    Protocol buffers (protobuf) have an interface description language (IDL) that describes the structure of the data.
    This IDL allows developers to serialize and deserialize data from byte-streams to structures in their language of choice.
    ...
    Developers can use the protobuf compiler to generate code for interfacing with these types.
...
## The way we talk about data types changes over time

If the only requirements were to define and use the data, our lives as software engineers would be easy.
The problem arises when we look at data types through a software development lifecycle (SDLC) lens.
The above types are a small snapshot of the data we might need as we add new features and this application changes.
Suppose we wanted to add a new field to `User` - an `email` so we can send them updates when our blog is published!

<Tabs>
  <TabItem label="TypeScript">

    For TypeScript, we can update the type and update areas where it's used.
  ...
  <TabItem label="SQLite">
    ...
    ```sql
    -- This CREATE isn't necessary if we assume we're still using the database from before
    -- For an existing database this won't run because EXISTS = true (NOT EXISTS = false)
    CREATE TABLE IF NOT EXISTS users (
      id TEXT PRIMARY KEY NOT NULL,
      name TEXT NOT NULL
    );
...
### The structure of the data often must change to accommodate data changes
...
- Protobuf advises that you don't do this since it's a breaking change that will break backward compatibility with old clients.
